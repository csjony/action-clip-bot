...
_foley_jobs: dict = {}       # async foley jobs (same pattern as video _jobs)
...
@app.post("/foley")
def start_foley(video: UploadFile = File(...), prompt: str = Form(""),
                negative_prompt: str = Form(""),
                duration: float = Form(6.0), steps: int = Form(25),
                cfg_strength: float = Form(4.5), seed: int = Form(42)):
    """Queue a self-hosted video-to-audio Foley job (free, no Replicate).

    Async like /generate: returns {"job_id"} immediately because the first
    call includes the one-time MMAudio weight load (minutes — far beyond any
    tunnel/proxy idle timeout). Poll /foley-status/{job_id}, then download
    the WAV via /foley-result/{job_id}.
    """
    import tempfile
    ...
    _foley_jobs[job_id] = {
        "status": "queued", "wav_path": None, "error": None,
        "created_at": time.time(),
        "params": {
            "in_path": in_path,
            "prompt": prompt or "cinematic sound effects",
    ...
    _threading.Thread(target=_run_foley_job, args=(job_id,), daemon=True).start()
    name = video.filename or "clip"
    print(f"[foley {job_id[:8]}] Queued: {prompt[:60]!r} for {name}")
    return {"job_id": job_id, "status": "queued"}


def _run_foley_job(job_id: str) -> None:
    """Background Foley worker: switch backend once, render, store WAV path."""
    import gc
    import torchaudio
    from mmaudio.eval_utils import generate, load_video
    from mmaudio.model.flow_matching import FlowMatching
    with _generation_lock:
        job = _foley_jobs[job_id]
        job["status"] = "running"
        p = job["params"]
        try:
            try:
            ...
            video_info = load_video(p["in_path"], p["duration"])
            clip_frames = video_info.clip_frames.unsqueeze(0)
            sync_frames = video_info.sync_frames.unsqueeze(0)
            rng = torch.Generator(device=device)
            rng.manual_seed(p["seed"])
            fm = FlowMatching(min_sigma=0, inference_mode="euler", num_steps=p["steps"])
            ...
            with torch.no_grad():
                audios = generate(
                    clip_frames, sync_frames, [p["prompt"]],
                    negative_text=[p["negative_prompt"]],
                    feature_utils=fu, net=net, fm=fm, rng=rng,
                    cfg_strength=p["cfg_strength"])
            out_wav = os.path.join(os.path.dirname(p["in_path"]), "foley.wav")
            torchaudio.save(out_wav, audios.float().cpu()[0], seq_cfg.sampling_rate)
            job["wav_path"] = out_wav
            job["status"] = "done"
            print(f"[foley {job_id[:8]}] done! {video_info.duration_sec:.1f}s -> {out_wav}")
        except Exception as exc:
            import traceback
            tb = traceback.format_exc()
            print(tb)
            print(f"[foley {job_id[:8]}] failed: {exc}")
            job["error"] = str(exc)
            job["status"] = "failed"
        finally:
            gc.collect()
...
@app.get("/foley-status/{job_id}")
def foley_status(job_id: str):
    """Poll Foley job status: queued | running | done | failed."""
    job = _foley_jobs.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail=f"Foley job {job_id!r} not found")
    return {"job_id": job_id, "status": job["status"], "error": job.get("error")}


@app.get("/foley-result/{job_id}")
def foley_result(job_id: str):
    """Download the generated WAV once status == 'done'."""
    job = _foley_jobs.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail=f"Foley job {job_id!r} not found")
    if job["status"] == "failed":
        raise HTTPException(status_code=500, detail=job.get("error", "Foley failed"))
    if job["status"] != "done":
    ...
    return FileResponse(wav_path, media_type="audio/wav", filename="foley.wav")
